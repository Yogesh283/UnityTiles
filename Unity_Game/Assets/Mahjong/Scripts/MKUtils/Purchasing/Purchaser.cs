...
#pragma warning restore CS0618
        ...
        {
#if !UNITY_EDITOR
            if (Mkey.Network.PlayStorePurchaseBridge.ShouldVerifyOnServer())
            {
                Mkey.Network.PlayStorePurchaseBridge.HandlePurchase(this, args);
                return PurchaseProcessingResult.Pending;
            }
#endif
            return CompletePurchaseLocally(args.purchasedProduct);
        }

        public void CompletePendingPurchase(Product product, ShopThingData prod)
        {
            m_StoreController.ConfirmPendingPurchase(product);
            if (prod != null)
                GoodPurchaseEvent?.Invoke(prod.kProductID, prod.name);
            else
                Debug.Log("[Purchaser] Server-verified purchase for unknown product: " + product.definition.id);
        }

        public void FailPendingPurchase(Product product, ShopThingData prod)
        {
            if (prod != null)
                FailedPurchaseEvent?.Invoke(prod.kProductID, prod.name);
            Debug.Log("[Purchaser] Pending purchase rejected by server verification.");
        }

        private PurchaseProcessingResult CompletePurchaseLocally(Product product, ShopThingData prod = null)
        {
            prod ??= GetProductById(product.definition.id);
            if (prod != null)
            {
        ...
        public ShopThingData GetShopItem(string id)
        {
