...
def join_duel(token: str, api_base: str) -> tuple[int, dict | str]:
    return post_json("tournaments/join", {"tournament_id": "duel_1v1"}, token, api_base=api_base)


def verify_sequential(api_base: str, suffix: str) -> int:
    token1 = guest_login(f"duel_verify_p1_{suffix}", api_base)
    status1, join1 = join_duel(token1, api_base)
    if status1 != 200:
    ...
    print(f"PASS P1 join room_id={room1} seed={seed1} status={join1.get('status')} players={join1.get('player_count')}")
...
def verify_concurrent(api_base: str, suffix: str) -> int:
    ...
    token1 = guest_login(f"duel_race_p1_{suffix}", api_base)
    token2 = guest_login(f"duel_race_p2_{suffix}", api_base)
